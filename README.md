...
A little Obsidian plugin that adds simple, elegant inline tooltips to your notes using a customizable syntax.
...
- **Simple Syntax**: Just wrap your text like this: `{Term}{Definition}`.
- **Live Preview Support**:
    - When your cursor is **away**, it renders as a clean, underlined term. Hovering shows the tooltip.
    - When your cursor is **on** the term, it expands to show the source code `{Term}{Definition}` for easy editing.
...
If you like this color scheme, you can download `sakura.json` from the [Releases](https://github.com/yunagi7/obsidian-inline-tooltip/releases) page and move it to `.obsidian/plugins/obsidian-inline-tooltip` and rename it to `data.json`.
...
This project was born out of a desire for a cleaner writing experience. I initially implemented this functionality using a CSS snippet and manually typing `<span class="p-tooltip" data-tip="Definition">Term</span>` tags. 
While effective, writing raw HTML inside Markdown quickly became tedious and cluttered the notes.

I built this plugin to solve that friction. It abstracts the complex HTML structure behind a simple `{Term}{Definition}` syntax, allowing you to keep your Markdown clean while retaining the rich tooltip functionality.
